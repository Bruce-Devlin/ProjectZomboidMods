...
function DaysUntilWinterUI:render()
    ...
    local text
    if days then
        local localization = getText("UI_DaysUntilWinter_Days")
        text = string.format("%s: %d", localization, days)
    else
        text = getText("UI_DaysUntilWinter_Winter")
    end
